...
Adding the census class to the census data.
...
Creating the census summary function that will produce means and standard deviations numeric variables as well as counts for categorical variables. Within the summary function, we will need to determine the two of the three arguments (the user provided numeric and categorical variables respectively).

```{r}
summary.census <- function(tibble, num_var = NULL, cat_var = NULL){
  #Checking for census class
  if (! "census" %in% class(tibble)) {
    stop("Error: Does not have census class")
  }
  #Creating weight vector
  weight_vector <- tibble$PWGTP
  
  #Defining numeric variable (user-selection)
  if (is.null(num_var)) {
    
    #Default (all numeric colums besides PWGTP)
    numeric_vars <- names(dplyr::select(tibble, where(is.numeric)))
    numeric_vars <- numeric_vars[numeric_vars != "PWGTP"]
  ...
  if (is.null(cat_var)) {
    cat_vars <- names(dplyr::select(tibble, where(is.character)))

...
The purpose of all of the code that is included in this narrative is to highlight examples of and to utilize functions created in the function code file of this project to both query the US Census API, as well as output the information contained within into a convenient tibble format, while allowing the user to specify several endpoints of interest within the requested region/geographical subset. We start by creating a function that takes any general URL (In this example the URL for the 2022 Census Microdata API from Census.gov) and then outputting the result of the GET() function as our URL_id object. This function will be utilized within future functions to quickly transform the URL that has been built to the user's specifications. After looking at its structure we can see that the raw data is in the content element. 
```{r}
